...
void sock_enable_resuseaddr(int fd) {
    int enable = 1;
...
int sock_connect(int fd, ipv4::endpoint const& ep) {
    ...
    if (!r || errno == EAGAIN || errno == EINPROGRESS) {
        return r;
    }

    throw ipv4::error(std::strerror(errno));
}
...
int sock_geterr(int fd) {
    ...
    int option_len = 8;
    int r = getsockopt(fd, SOL_SOCKET, SO_ERROR, (void*)&option_value,
...
socket::socket(io_api::io_context& ctx, endpoint const& ep, callback_t const& on_disconnect
        ...
        : socket(ctx, unique_fd(sock_create(AF_INET, SOCK_STREAM, 0)), {}, {}, {}) {
